  ...
  adjustQuickCheckTests (* 10) $
    testGroup
      ...
      [ testGroup
          ...
          , testCase
              "a bare announcement of an already-held EbHash still registers and notifies its new point (EB-hash collision, announcement path)" $ do
              ...
              runCmdsAndCollectAcquiredTxPoints cmds
                @?= Right [pointOf [0, 1] 5, pointOf [0, 1] 8]
          , testCase
              "a bare offer of an already-held EbHash still registers and notifies its new point (EB-hash collision, offer path)" $ do
              ...
              runCmdsAndCollectAcquiredTxPoints cmds
                @?= Right [pointOf [0, 1] 5, pointOf [0, 1] 8]
          , testCase
              "a second point sharing an already-held EbHash still traces TraceLeiosBlockTxsAcquired (EB-hash collision, kernel-trace gap)" $ do
